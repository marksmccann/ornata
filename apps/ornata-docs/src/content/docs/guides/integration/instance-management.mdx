...
<Aside title="Tip" type="note">
    Use `getInstance()` inside tracking or utility code where a missing
    component means an absolute system failure. Use `findInstance()` when
    checking for components that are optional or conditionally present on the
    page.
</Aside>
