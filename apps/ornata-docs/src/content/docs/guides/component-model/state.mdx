...
1. Component definition defaults
2. Root element `data-*` attributes
3. `initialState` passed to `mount()` [Highest Priority]
...
- Triggers UI render callbacks
- Fires matching `watch` hooks
- Notifies external state listeners
...
state: {
    count: { default: 0, type: Number },
    token: { private: true }, // `instance.state.token` equals `undefined` externally
    label: { default: "Clicks", readonly: true }, // `instance.state.label = 'New'` errors and is ignored
}
```
