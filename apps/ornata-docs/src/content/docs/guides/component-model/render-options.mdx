...
<Aside type="caution" title="Use trusted HTML only">
    `html` writes directly to `innerHTML`. Only use it with trusted content or content you have sanitized first. If you only need to show plain text, prefer `text` instead.
</Aside>
